...
function Landing() {
  ...
  const fetchMovies = async (searchQuery, currentPage, selectedType) => {
    ...
    try {
      const data = await searchMovies(trimmedQuery, currentPage, selectedType);
  ...
  useEffect(() => {
    ...
    const selectedType = searchParams.get("type") || "movie";

    setQuery(searchQuery);
    setPage(currentPage);
    setType(selectedType);
    fetchMovies(searchQuery, currentPage, selectedType);
  }, [searchParams]);

  const updateSearchParams = (searchQuery, currentPage, selectedType) => {
    const trimmedQuery = searchQuery.trim();
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    updateSearchParams(query, 1, type);
  };
      ...
      {movies.length > 0 ? (
        <>
          ...
          <div className="movie_section">
            <div className="container">
              <h1 className="section_title loading_title">Movie Lists</h1>
              {isLoading ? (
